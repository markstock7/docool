...
function applyNamespace(docletOrNs, tag) {
    if (typeof docletOrNs === 'string') { // ns
        tag.value = jsParser.name.applyNamespace(tag.value, docletOrNs);
    } else { // doclet
        if (!docletOrNs.name) {
            return; // error?
        }

        docletOrNs.longname = jsParser.name.applyNamespace(docletOrNs.name, tag.title);
    }
}
...
function parseTypeText(text) {
    var tagType = tag.type.parse(text, false, true);
...
var baseTags = exports.baseTags = {
    ...
    class: {
        onTagged: function(doclet, tag) {
                ...
                if (tag.originalTitle === 'class') {
                    ...
                    if ((looksLikeDesc || /@construct(s|or)\b/i.test(doclet.comment)) && !/@classdesc\b/i.test(doclet.comment)) {
    ...
    default: {
        onTagged: function(doclet, tag) {
            var nodeToValue = jsParser.astnode.nodeToValue;
    ...
    global: {
        mustNotHaveValue: true,
        onTagged: function(doclet, tag) {
            doclet.scope = jsParser.name.SCOPE.NAMES.GLOBAL;
    ...
    lends: {
        onTagged: function(doclet, tag) {
            doclet.alias = tag.value || jsParser.name.LONGNAMES.GLOBAL;
    ...
    memberof: {
        ...
        onTagged: function(doclet, tag) {
            if (tag.originalTitle === 'memberof!') {
                ...
                if (tag.value === jsParser.name.LONGNAMES.GLOBAL) {
...
/**
 * 向当前的字典中，注册合适的tag, jsdoc支持两种 js 和 closureTags,
 * elephant 目前只支持一种
 */
exports.defineTags = function(dictionary) {
    addTagDefinitions(dictionary, baseTags);
}