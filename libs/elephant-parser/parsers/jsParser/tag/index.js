...
var Tag = exports.Tag = function(tagTitle, tagBody, meta) {
    ...
    this.title = jsParser.tag.dictionary.normalise(this.originalTitle);

    tagDef = jsParser.tag.dictionary.lookUp(this.title);
    trimOpts = {
    ...
    jsParser.tag.validator.validate(this, tagDef, meta);
};
...
function parseType(tag, tagDef, meta) {
    try {
        return jsParser.tag.type.parse(tag.text, tagDef.canHaveName, tagDef.canHaveType);
    } catch (e) {
        // util.logger.error(
